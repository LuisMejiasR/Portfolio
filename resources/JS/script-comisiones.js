...
window.addEventListener("DOMContentLoaded", (e) => {
    ...
    boton.addEventListener("click", (evento) => {
        ...
        try{
            ...
            let MotivoComision = getMotivoComision();
...
function getMotivoComision() {
    //a través de esta variable tomo con selectores CSS todos los "input con nombre motivo-contacto que estén seleccionados"
    let inputMotivoComision = document.querySelectorAll("input[name='tipo-comision']:checked");
    //creo un arreglo para poder irlos sumando
    let arrMotivosComision = [];

    //recorro los input de motivos y le hago push al arreglo creado
    for (let i = 0; i < inputMotivoComision.length; i++){
        //creo la variable con el motivo seleccionado
        const motivo = inputMotivoComision[i].value;
        //para poderla pushear al arreglo
        arrMotivosComision.push(motivo);
    }

    if (inputMotivoComision.length < 1){
        throw new Error("You must choose at least one type of commission");
    }

    //regreso el arreglo completo
    return arrMotivosComision;
}
...
async function guardarFormularioLleno (formularioLleno){
    ...
    const url = baseUrl + "/comisiones.json";
    //metodo async await
